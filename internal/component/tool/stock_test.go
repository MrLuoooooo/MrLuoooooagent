...
func newTestCollector(dir string) *stock.Collector {
	logger := zap.NewNop()
	cache := stockcache.NewMemoryCache(logger)
	store := stockstore.NewFileStore(dir, logger)
	return stock.NewCollector(nil, cache, store, logger)
}

func TestStockRealtimeTool_Info(t *testing.T) {
	st := NewStockRealtimeTool(newTestCollector(t.TempDir()))
...
func TestStockRealtimeTool_ReadFileSuccess(t *testing.T) {
	dir := t.TempDir()
	stockDir := filepath.Join(dir, "stocks", "realtime")
	...
	st := NewStockRealtimeTool(newTestCollector(dir))
...
func TestStockKLineTool_Info(t *testing.T) {
	sk := NewStockKLineTool(newTestCollector(t.TempDir()))
...
func TestStockKLineTool_ReadSuccess(t *testing.T) {
	dir := t.TempDir()
	histDir := filepath.Join(dir, "stocks", "historical", "day")
	os.MkdirAll(histDir, 0755)
	klines := []api.KLineData{
	...
	os.WriteFile(filepath.Join(histDir, "sh600519.json"), d, 0644)

	sk := NewStockKLineTool(newTestCollector(dir))
